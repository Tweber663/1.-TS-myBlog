{
  ...
  const generateTitleLinks = (customSelector: string = ''): void => {
    ...
    for (let article of articles) {
      ...
      const articleID: string = article.getAttribute('id') as string;
  ...
  const generateTags = (): void => {
    ...
    for (let article of articles) {
      ...
      const tagWrapper = article.querySelector(selectorArticleTags) as HTMLLinkElement;

          /* prepare variable for storing all the tag links */
      let html: string = '';

      /* get info about tags from data-tags attribute */
      const dataTag: string | null = article.getAttribute('data-tags')

      /* split tags into array */
      const tagsArray: string[] | null = dataTag.split(' ');
  ...
  const generateAuthors = (): void => {
    ...
    const authorList = document.querySelector(selectorAuthorsList) as HTMLLinkElement
